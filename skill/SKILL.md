...
# Nightdrop — Hermes Mailbox Broker and Outbound Action Gate

Nightdrop gives Hermes bounded access to named Gmail and Outlook Inbox profiles while keeping App Passwords and OAuth tokens inside a separate service, and lets Hermes **propose** outbound actions for deterministic human-approved execution.
...
/usr/local/bin/nightdrop-mailbox list --profile PROFILE --unread --limit 20
...
4. Do **not** claim the action was sent. It is only pending until Nightdrop reports success to the human.
...
```bash
/path/to/nightdrop/skill/scripts/draft-email.sh \
...
The helper uses `sg nightdrop-inbox` to drop the file into `/opt/nightdrop/drafts/inbox`. If the production path differs, set `NIGHTDROP_INBOX` before running the helper.
...
- `from` is ignored by Nightdrop; the configured provider sender is shown in the approval preview and used for execution.
...
- “Waiting for approval in Nightdrop Telegram.”
