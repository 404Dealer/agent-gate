...
const PASS_REFERENCE = /^\$\{PASS:(agent-gate\/[a-z0-9][a-z0-9-]{0,127})\}$/;
...
const assertPrivateConfig = async (configPath: string): Promise<void> => {
  ...
  if (uid === undefined || uid === 0) {
    throw new Error('Mailbox cleanup must run as the isolated agentgate user');
  }
  ...
  if (stat.uid !== uid || (stat.mode & 0o777) !== 0o600) {
    throw new Error('Agent-gate config must be owned by the cleanup user with mode 0600');
  }
};
...
export async function loadGmailCleanupCredentials(
  ...
  const passExecutable = env.AGENT_GATE_PASS_BIN;
  ...
  try {
    ...
    throw new Error('Could not read the private agent-gate configuration');
  }
  const root = z.object({ providers: z.record(z.unknown()) }).safeParse(document);
  if (!root.success) throw new Error('Agent-gate configuration is missing providers');
